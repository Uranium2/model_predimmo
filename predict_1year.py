...
MODEL_ARTIFACT = "2020-07-08_model_predimmo_1year.h5"
MODEL_NAME = SAVE_MODEL_DIR + MODEL_ARTIFACT


def predict_classification_1year():
  def create_conn():
  ...
  def prediction_to_csv(X_init, y):
    ...
    X_init['prediction_1'] = y
    df_pred = X_init[['code_postal', 'prediction_1']].groupby(['code_postal']).mean().round()
    df_pred = df_pred.reset_index()

    df_pred = df_pred.astype({"prediction_1": int})
    
    print("PREDICTIONS FOR EACH DEPARTMENTS\n", df_pred)

    df_pred.to_csv(DATA_PREDICTIONS + "prediction_1year.csv", index=False)
